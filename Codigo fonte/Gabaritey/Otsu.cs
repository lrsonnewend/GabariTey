...
namespace Gabaritey
{
    class Otsu
    {
        // a função é usada para calcular os valores de Q na equação
        ...
        // a função é usada para calcular os valores médios na equação (mu)
        ...
        // simplesmente calcula o histograma da imagem
        ...
        public int getOtsuThreshold(Bitmap bmp)
        {
            ...
            unsafe
            {
                ...
                // superar todos os valores t possíveis e maximizar a variância da classe
                for (k = 1; k != 255; k++)
                {
                    p1 = Px(0, k, hist);
                    p2 = Px(k + 1, 255, hist);
        ...
        //Rotina simples para converter em escala de cinza
        public void Convert2GrayScaleFast(Bitmap bmp)
        {
            BitmapData bmData = bmp.LockBits(new Rectangle(0, 0, bmp.Width, bmp.Height),
                    ImageLockMode.ReadWrite, PixelFormat.Format24bppRgb);
            //Unsafe é um codigo nao seguro usado na declaração de um tipo ou membro
            unsafe
            {
                byte* p = (byte*)(void*)bmData.Scan0.ToPointer();
        ...
        public void threshold(Bitmap bmp, int thresh)
        {
            ...
            unsafe
            {
                ...
                //Percorre a imagem, se o valor for acima da escala de cinza troca para preto, se nao troca para branco
                for (int i = 0; i < h; i++)
                {
                    byte* row = &p[i * ws];
                    for (int j = 0; j < w * 3; j += 3)
        ...
        public void analisar(ref Bitmap bmp)
        {
            BitmapData bmData = bmp.LockBits(new Rectangle(0, 0, bmp.Width, bmp.Height),
                    ImageLockMode.ReadWrite, PixelFormat.Format24bppRgb);
            unsafe
